...
from foressment_ai import DataScaler, AIForecaster, ForecastEstimator, \
    Logger, DataLoaderExtractor
from foressment_ai import ForessmentData as PDATA
...
def example_forecaster_model_training(logname='example'):
    """
    Testing Forecaster module methods.
...
def example_forecasting(logname='example',
                        apriori=True):
    """
    Testing Forecaster module methods
...
def example_join_test_smart_crane():
    ...
    for mode in range(1, 9):
        for test_name, use_extractor in {'c' + str(mode) + '_forecaster_extractor0': False,
                                     'c' + str(mode) + '_forecaster_extractor1': True}.items():

            Loader = DataLoaderExtractor(test_name=test_name)
            PDATA.features_matrix = Loader(dataset_name, mode=mode, use_extractor=use_extractor)
            ...
            for ex in [str(i) + '_' for i in range(3)]:
                ...
                if ex == '0_':
                    result = result.append(example_forecaster_model_training(logname=logname), ignore_index=True)
                if ex == '1_':
                    result = result.append(example_forecasting(logname=logname), ignore_index=True)
                if ex == '2_':
                    result = result.append(example_forecasting(logname=logname, apriori=False), ignore_index=True)
    result = result.sort_values(by='test').reset_index(drop=True)
    result.to_csv('forecaster_logs/join_test_' + dataset_name + '.csv', index=False, sep=';', decimal=',')
...
def example_join_test_hai():
    ...
    dataset_name = 'hai'
    for test_name, use_extractor in {'forecaster_extractor0': False,
                                 'forecaster_extractor1': True}.items():

        Loader = DataLoaderExtractor(test_name=test_name)
        PDATA.features_matrix = Loader(dataset_name, use_extractor=use_extractor)
        PDATA.features_names = Loader.features_names
        PDATA.forecasting_model_path = Loader.forecasting_model_path
        PDATA.normalization_model_path = Loader.normalization_model_path
