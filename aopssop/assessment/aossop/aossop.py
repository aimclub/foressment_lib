...
class _NNClassifier:
    ...
    Атрибуты:
        model: Модель глубокой нейронной сети (keras.models.Sequential).
        out_size: Количество нейронов на выходном слое (int).
        plot: Булев параметр, задающий вывод графика обучения нейронной сети (bool). Default=True.
        num_epochs: Количество эпох обучения (int). Default=10.
    """
    ...
    def fit(self, x_train, y_train):
        ...
        self.history = self.model.fit(x_train, y_train, epochs=self.num_epochs, batch_size=32, verbose=1, validation_split=0.2)
        if self.plot:
            self.draw_plot()
        return self
...
class SAIClassifier:
    ...
    Атрибуты:
        cls_type: тип классификатора (str).
        classifier: Объект классиификатора (keras.models.Sequential и sklearn.*).
    """
...
class FormatDetector:
    ...
    Атрибуты:
        n: Количество полей в записи (int).
        d: Разделитель полей в записи (str).
    """
...
class DataLoader:
    ...
    Атрибуты:
        n: Количество полей в записи (int).
        d: Разделитель полей в записи (str).
        features: Признаки (numpy.ndarray).
        labels: Метки классов (one-hot кодирование) (numpy.ndarray).
        num_labels: Метки классов (numpy.ndarray).
    """
...
class ClsEstimator:
    ...
    Атрибуты:
        ...
        labels: Метки классов (one-hot кодирование) (numpy.ndarray).
        num_labels: Метки классов (numpy.ndarray).
        classifiers: Классификаторы (list).
    """
    ...
    def estimate(self, print_metrics=True):
        ...
        for cls in self.classifiers:
            ...
            for (f, l, n) in [(train_features, train_labels, 'training'), (test_features, test_labels, 'testing')]:
                ...
                res[cls.cls_type][n + '_sample'] = {'size': len(l), 'precision': precision, 'recall': recall, 'fscore': fscore, 'accuracy': accuracy}
        return res
