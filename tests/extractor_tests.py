...
class ExtractorTest(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        super(ExtractorTest, self).__init__(*args, **kwargs)
        ExtractorTest.n = 1

    def setUp(self):
        print('Test {} BEGIN'.format(ExtractorTest.n))

    def tearDown(self):
        print('Test {} END'.format(ExtractorTest.n))
        ExtractorTest.n += 1

    def test_regression_coefficient(self):
        print(inspect.stack()[0][3])
        message = 'The regression coefficient is calculated incorrectly'
        self.assertEqual(round(RulesExtractor().calculate_regression_coefficient(nA=200, nB=200, nAB=100, N=1000), 3), 0.375,
                         message)

    def test_klosgen_measure(self):
        print(inspect.stack()[0][3])
        message = 'The klosgen measure is calculated incorrectly'
        self.assertEqual(round(RulesExtractor().calculate_klosgen_measure(nA=200, nB=200, nAB=100, N=1000), 3), 0.134,
                         message)
